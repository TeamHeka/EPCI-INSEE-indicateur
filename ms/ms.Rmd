...
The vaccination dataset for mainland France encompasses about `r round(pops[nrow(pops), 1] / 10^6, 1)` million individuals (median district size `r round(popSizeDistrib["50%"])`, interquartile range `r round(popSizeDistrib["25%"])`--`r round(popSizeDistrib["75%"])`). The vaccination data are available by age class: 00--19, 20--39, 40--54, 55--64, 65--74, 75 and over. Population sizes for each locality and each age class are also provided. These population sizes are updated monthly to account for individuals aging, potential residential moves, and deaths. XXX TODO: Demander pourquoi les tailles de pop sont constantes au cours du temps
...
Socio-economic data are provided by the French national statistics institute (INSEE), and are available at the same administrative levels as the vaccination data. We selected the most recent dataset available (year 2018). The different predictors are classified into `r length(insee.indicators.names)` categories (`r insee.indicators.names`).  
...
```{r figOverTime, fig.height=8, fig.width=4, fig.cap="Age-adjusted vaccination rates among adults, over time, by decile of each indicator (presented by a color gradient). The dates of announcements and implementations of the sanitary and vaccine pass are indicated near the horizontal axes.", echo = FALSE}
...
```{r figMap, fig.height=5, fig.width=4, fig.cap="Adult vaccination rates by district", echo = FALSE}
...
Our results, based on exhaustive national datasets, indicate that the French sanitary pass, and the later vaccine pass, did not solve Covid-19 vaccination heterogeneities, but instead crystallized them. Indicators most associated with vaccination rates were associated to poverty, immigration, and anti-establishment vote (or abstention). For instance, the odds for an adult to still be unvaccinated by the end of February 2021 are about `r round(maxOR.Unemployment_Benef, 1)` times higher when living in the districts with higher than median value share of income coming from unemployment benefits, and when living in the districts with lower than median value. 
