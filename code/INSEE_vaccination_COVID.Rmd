...
GEO_OM$lat01 <- (GEO_OM$latitude - min(GEO_OM$latitude, na.rm = T)) / (max(GEO_OM$latitude, na.rm = T) - min(GEO_OM$latitude, na.rm = T))
GEO_OM$lon01 <- (GEO_OM$longitude - min(GEO_OM$longitude, na.rm = T)) / (max(GEO_OM$longitude, na.rm = T) - min(GEO_OM$longitude, na.rm = T))

GEO_OM = GEO_OM %>%
  mutate(`SO-NE` = lat01+lon01,
         `NO-SE` = lon01-lat01)
...
GEO_OR = questionr::odds.ratio(GEO_fact$latitude, GEO_fact$taux_cumu_1_inj) %>%
  ...
  rbind(questionr::odds.ratio(GEO_fact$`SO-NE`, GEO_fact$taux_cumu_1_inj)) %>%
  rbind(questionr::odds.ratio(GEO_fact$`NO-SE`, GEO_fact$taux_cumu_1_inj)) %>%
  as.data.frame() %>%
  bind_cols(Variable = colnames(GEO_fact)[2:6]) %>%
  select(Variable, OR, `2.5 %`, `97.5 %`, p)
