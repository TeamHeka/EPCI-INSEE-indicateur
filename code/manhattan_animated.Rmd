...
read.csv("data/GEO_brut.csv") %>%
  select(-c(codgeo, taux_cumu_1_inj, SEXE.AGE3_B_POP_ENS_ENS, lat01, lon01, superficie)) %>%
  mutate(latitude = 41+35/60+30/3600+(51+2/60+18/3600-41-35/60-30/3600)*(latitude-6069383)/(7101616-6069383),
         longitude = -4-38/60-16/3600+(9+31/60+15/3600+4+38/60+16/3600)*(longitude+4+38/60+16/3600)/(1232484-131265)) %>%
  desctable(stats = list("N" = length, "Med" = median, "IQR" = IQR)) %>%
  pander()
```
