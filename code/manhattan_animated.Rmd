...
données électorales 2017 : https://www.data.gouv.fr/fr/datasets/election-presidentielle-des-23-avril-et-7-mai-2017-resultats-definitifs-du-1er-tour-par-communes/#resource-d282e53a-d273-425d-95bb-8a0d7632c79a-header

données électorales de paris 2017 : https://opendata.paris.fr/explore/dataset/elections-presidentielles-2017-1ertour/export/?disjunctive.id_bvote&disjunctive.num_circ&disjunctive.num_quartier&disjunctive.num_arrond&sort=-num_arrond
données électorales de marseille 2017 :
https://trouver.datasud.fr/dataset/82a6d91c-c81d-423c-9a4a-3f76d121c8ce/resource/03e2ef07-c2d0-41dd-b503-26910ecb15c3/download/marseille_presidentielles2017_tour1.csv
...
composition communale des EPCI : https://www.insee.fr/fr/information/2510634

Composition communale des communes nouvelles : https://www.insee.fr/fr/information/2549968

```{r, eval = F, echo = F, message = F, warning = F}
pres = "data/Presidentielle_2017_Resultats_Communes_T1_clean_def.csv"

pres_paris = read_csv2(url("https://opendata.paris.fr/explore/dataset/elections-presidentielles-2017-1ertour/download/?format=csv&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B")) %>%
  group_by(NUM_ARROND) %>%
  summarise(Inscrits = sum(NB_INSCR),
            Abstentions = Inscrits - sum(NB_VOTANT),
            Abstention = Abstentions / Inscrits,
            Blancs = sum(NB_BL),
            Nuls = sum(NB_NUL),
            Exprime = Inscrits - Abstentions - Blancs - Nuls,
            LePen = sum(`LE PEN Marine`) / Exprime,
            Melenchon = sum(`MÉLENCHON Jean-Luc`) / Exprime,
            Macron = sum(`MACRON Emmanuel`) / Exprime,
            Fillon = sum(`FILLON François`) / Exprime,
            DupontAignan = sum(`DUPONT-AIGNAN Nicolas`) / Exprime,
            Lassalle = sum(`LASSALLE Jean`) / Exprime,
            Hamon = sum(`HAMON Benoît`) / Exprime,
            Asselineau = sum(`ASSELINEAU François`) / Exprime,
            Poutou = sum(`POUTOU Philippe`) / Exprime,
            Arthaud = sum(`ARTHAUD Nathalie`) / Exprime,
            Cheminade = sum(`CHEMINADE Jacques`) / Exprime) %>%
  ungroup() %>%
  mutate(codgeo = as.character(75100 + NUM_ARROND), .after = NUM_ARROND) %>%
  select(-c(Inscrits, Abstentions, Blancs, Nuls, Exprime, NUM_ARROND))

pres_marseille = read_csv(url("https://trouver.datasud.fr/dataset/82a6d91c-c81d-423c-9a4a-3f76d121c8ce/resource/03e2ef07-c2d0-41dd-b503-26910ecb15c3/download/marseille_presidentielles2017_tour1.csv")) %>%
  group_by(ARRONDISSEMENT) %>%
  summarise(Inscrits = sum(NB_INSCRITS_THEORIQUES),
            Abstentions = Inscrits - sum(NB_VOTANTS),
            Abstention = Abstentions / Inscrits,
            Blancs = sum(NB_BLANCS),
            Nuls = sum(NB_NULS),
            Exprime = Inscrits - Abstentions - Blancs - Nuls,
            LePen = sum(NB_VOIX_CANDIDAT_02) / Exprime,
            Melenchon = sum(NB_VOIX_CANDIDAT_09) / Exprime,
            Macron = sum(NB_VOIX_CANDIDAT_03) / Exprime,
            Fillon = sum(NB_VOIX_CANDIDAT_11) / Exprime,
            DupontAignan = sum(NB_VOIX_CANDIDAT_01) / Exprime,
            Lassalle = sum(NB_VOIX_CANDIDAT_08) / Exprime,
            Hamon = sum(NB_VOIX_CANDIDAT_04) / Exprime,
            Asselineau = sum(NB_VOIX_CANDIDAT_10) / Exprime,
            Poutou = sum(NB_VOIX_CANDIDAT_06) / Exprime,
            Arthaud = sum(NB_VOIX_CANDIDAT_05) / Exprime,
            Cheminade = sum(NB_VOIX_CANDIDAT_07) / Exprime) %>%
  ungroup() %>%
  mutate(codgeo = as.character(13200 + ARRONDISSEMENT), .after= ARRONDISSEMENT) %>%
  select(-c(Inscrits, Abstentions, Blancs, Nuls, Exprime, ARRONDISSEMENT))

pres_lyon = read.csv2("data/pres_lyon.csv") %>%
  mutate(codgeo = as.character(codgeo),
         Abstention = Abstentions / Inscrits,
         LePen = LePen / Exprime,
         Melenchon = Melenchon / Exprime,
         Macron = Macron / Exprime,
         Fillon = Fillon / Exprime,
         DupontAignan = DupontAignan / Exprime,
         Lassalle = Lassalle / Exprime,
         Hamon = Hamon / Exprime,
         Asselineau = Asselineau / Exprime,
         Poutou = Poutou / Exprime,
         Arthaud = Arthaud / Exprime,
         Cheminade = Cheminade / Exprime) %>%
  select(-c(Inscrits, Abstentions, Exprime))

Presidentielle_2017 = read_csv(pres) 

Intercom <- read_excel("~/nas_backup_scripts_elr/INSEE_API/data/Intercommunalite-Metropole_au_01-01-2021.xlsx", 
                       sheet = "Composition_communale", skip = 5)

EPCI_PLM = c(200054781, 200046977, 200054807)
EPCI_domtom = c(249710062, 200044691, 249710070, 200018653, 200041507, 249710047, #Guadeloupe
                249720061, 249720053, 200041788, #Martinique
                249730045, 249730052, 249730037, 200027548, #Guyane
                249740077, 249740119, 249740093, 249740085, 249740101, #LaReunion
                200060457, 200060465, 200050532, 200059871, 200060473) #Mayotte

CN17 = read_excel("data/communes_nouvelles_2017.xls")
CN18 = read_excel("data/communes_nouvelles_2018.xls")
CN19 = read_excel("data/communes_nouvelles_2019.xls")
CN20 = read_excel("data/communes_nouvelles_2020.xlsx")
CN21 = read_excel("data/communes_nouvelles_2021.xlsx")

CN = CN18 %>% select(DepComN, DepComA) %>%
  bind_rows(CN17 %>% select(DepComN, DepComA)) %>%
  bind_rows(CN19 %>% select(DepComN, DepComA)) %>%
  bind_rows(CN20 %>%
              mutate(DepComN = as.character(DepComN),
                     DepComA = as.character(DepComA)) %>%
              select(DepComN, DepComA)) %>%
  bind_rows(CN21 %>% select(DepComN, DepComA))

base_codes = EMP_brut %>%
  select(codgeo)

PRE_brut = Presidentielle_2017 %>%
  select(CodeInsee, Inscrits, Abstentions, Votants, Blancs, Nuls, `LE PEN`,
         MÉLENCHON, MACRON, FILLON, `DUPONT-AIGNAN`, LASSALLE, HAMON,
         ASSELINEAU, POUTOU, ARTHAUD, CHEMINADE) %>%
  mutate(CodeInsee = if_else(CodeInsee == "27676", "27058", CodeInsee)) %>%
  left_join(CN, by = c("CodeInsee" = "DepComA")) %>%
  mutate(DepComN = if_else(is.na(DepComN), CodeInsee, DepComN)) %>%
  group_by(DepComN) %>%
  summarise(Inscrits = sum(Inscrits),
            ...
            Nuls = sum(Nuls),
            LePen = sum(`LE PEN`),
            Melenchon = sum(MÉLENCHON),
            Macron = sum(MACRON),
            Fillon = sum(FILLON),
            DupontAignan = sum(`DUPONT-AIGNAN`),
            Lassalle = sum(LASSALLE),
            Hamon = sum(HAMON),
            Asselineau = sum(ASSELINEAU),
            Poutou = sum(POUTOU),
            Arthaud = sum(ARTHAUD),
            Cheminade = sum(CHEMINADE)) %>%
  ungroup() %>%
  anti_join(Intercom %>%
              filter(EPCI %in% EPCI_domtom), by = c("DepComN" = "CODGEO")) %>%
  left_join(Intercom %>%
              filter(!(EPCI %in% EPCI_PLM)), by = c("DepComN" = "CODGEO")) %>%
  mutate(EPCI = if_else(is.na(EPCI), DepComN, EPCI)) %>%
  group_by(EPCI) %>%
  summarise(Inscrits = sum(Inscrits),
            Abstentions = sum(Abstentions),
            Abstention = Abstentions / Inscrits,
            ...
            Nuls = sum(Nuls),
            Exprime = (Inscrits - Abstentions - Blancs - Nuls),
            LePen = sum(LePen) / Exprime,
            Melenchon = sum(Melenchon) / Exprime,
            Macron = sum(Macron) / Exprime,
            Fillon = sum(Fillon) / Exprime,
            DupontAignan = sum(DupontAignan) / Exprime,
            Lassalle = sum(Lassalle) / Exprime,
            Hamon = sum(Hamon) / Exprime,
            Asselineau = sum(Asselineau) / Exprime,
            Poutou = sum(Poutou) / Exprime,
            Arthaud = sum(Arthaud) / Exprime,
            Cheminade = sum(Cheminade) / Exprime) %>%
  ungroup() %>%
  select(-c(Inscrits, Abstentions, Blancs, Nuls, Exprime)) %>%
  rename(codgeo = EPCI) %>%
  plyr::rbind.fill(pres_lyon, pres_marseille, pres_paris) %>%
  filter(!is.na(codgeo)) %>%
  inner_join(base_codes %>% mutate(codgeo = as.character(codgeo)))

write_csv(PRE_brut, "data/PRE_brut.csv")

PRE_fact = PRE_brut %>%
  mutate(across(.cols = -codgeo, .fns = binarisation))

write_csv(PRE_fact, "data/PRE_fact.csv")
```
...
PRE_fact = read.csv("data/PRE_fact.csv")
...
make_data_covid = function(wk, variable = "taux_cumu_1_inj", classe_age = "tous"){
  ...
  # homogénéisation de la population de l'EPCI par classe d'age
  # la référence est la structure de la population francaise à la date
  if (classe_age == "tous"){
    vacc = unique(vacc) %>%
      # Uniquement les codes géographiques des territoires de métropole sans les codes des métropole de Paris Lyon et marseille pour évides les comptes doubles
      filter(codgeo %in% base_codes) %>% 
      # Transformation de la population en numérique
      mutate(population_carto = as.numeric(population_carto)) 
    # Calcul de la population francaise totale
    totPop = sum(vacc$population_carto)
    # Calcul des proportions de chaque classe d'age dans la population francaise
    props = vacc %>%
      group_by(classe_age) %>%
      summarise(proportionPop = sum(population_carto) / totPop) %>%
      ungroup()
    # Calcul des taux de vaccination homogénéisés par structure de population
    vacc = vacc %>%
      # Jointure des taux de vaccination au tableau de vaccination.
      # La clé de jointure est la classe d'age
      inner_join(props) %>%
      # Multiplication du taux de vaccination par le taux de chaque classe d'age dans la population francaise
      mutate(taux_cumu_1_inj = taux_cumu_1_inj * proportionPop) %>%
      # Sommation des nouveaux taux de vaccination au sein de chaque ECPI
      group_by(codgeo) %>%
      summarise(taux_cumu_1_inj = sum(taux_cumu_1_inj)) %>%
      ungroup()
  }
  ...
  if (variable == "taux_cumu_1_inj"){ # dans la pratique on sera toujours dans ce cas
    ...
    PRE_1107 = PRE_fact %>% select(-taux_cumu_1_inj) %>% inner_join(vacc) %>% mutate(taux_cumu_1_inj = as.factor(binarisation(as.numeric(taux_1_inj, 0.25))))
  }
  ...
  PRE_OR_1107 = questionr::odds.ratio(PRE_1107$Abstention, PRE_1107$taux_cumu_1_inj) %>%
    rbind(questionr::odds.ratio(PRE_1107$LePen, PRE_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Melenchon, PRE_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Macron, PRE_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Fillon, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$DupontAignan, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Lassalle, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Hamon, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Asselineau, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Poutou, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Arthaud, GEO_1107$taux_cumu_1_inj)) %>%
    rbind(questionr::odds.ratio(PRE_1107$Cheminade, GEO_1107$taux_cumu_1_inj)) %>%
    as.data.frame() %>%
    bind_cols(Variable = colnames(PRE_1107)[2:13]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  rownames(GEO_OR_1107) = NULL
  ...
  DF_OR_1107 = LOG_OR_1107 %>% mutate(theme = "Housing", .after = Variable) %>%
    ...
    bind_rows(REV_OR_1107 %>% mutate(theme = "Income", .after = Variable)) %>%
    bind_rows(ACT_OR_1107 %>% mutate(theme = "Activity", .after = Variable)) %>%
    bind_rows(FOR_OR_1107 %>% mutate(theme = "Education", .after = Variable)) %>%
    ...
    bind_rows(PRE_OR_1107 %>% mutate(theme = "Elections", .after = Variable)) %>%
    group_by(theme) %>%
...
### **Semaine 35** : Après la mise en place du pass sanitaire
...
### **Semaine 52** : Après l'annonce du pass vaccinal

```{r, s52, warning = F, echo = F, message = F}
read_csv2(url("https://datavaccin-covid.ameli.fr/explore/dataset/donnees-de-vaccination-par-epci/download/?format=csv&refine.semaine_injection=2021-52&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B"),
          col_type = list(
            date_reference = col_date(format = ""),
            semaine_injection = col_character(),
            epci = col_double(),
...
## thématique vote politique 2017

```{r, echo = F, message = F, warning = F}
read.csv("data/PRE_brut.csv") %>%
  desctable(stats = list("N" = length, "Med" = median, "IQR" = IQR)) %>%
  pander()
```

```{r, echo = F, message = F, warning = F}
data_b = read_csv("data/PRE_brut.csv")
...
Les variables ne sont pas corrélées.
...
```{r, echo = F, message = F, warning = F}
colour = c("#13124e", "#ffb0af", "#2a3080", "#f57969", "#6d5aa1", "#d11d11", "#9c9bdb", "#f49600", "#bb749a", "#ffb92d")
...
```{r}
l_indicator = c("noTransport_toWk", "1564_Retired_amg_NW", "SingleParent_HH", "80+_amg_Single")
```


```{r, eval = F, echo = F}
inds = c("Wk_Res_County-Municipality", "Wk_Res_Country-Region",
         "15+_MenOnWomen_NoDiploma", "15+_VocationalGraduate_amg_WF",
         "Macron", "Melenchon",
         "1564_OtherInactive_amg_NW", "2554_women_Workforce_rate",
         "OtherNoFamily_HH", "SingleParent_HH",
         "latitude",
         "1ry_res_FreeAcc",
         "Immigrant",
         "Unemployment_Benef", "Interdecile_91_Ratio",
         "15+_CraftsBusinessShopkeeper_amg_POP", "15+_women_OtherNoActivity_amg_wPOP")

data %>%
  filter(theme == "Population" & week == 52) %>%
  filter(`max_OR-inv_OR` == max(`max_OR-inv_OR`))

data %>%
  filter(Variable %in% inds) %>%
  mutate(influence = if_else(OR >1, "OR", "1/OR"),
         OR = round(if_else(influence == "OR", OR, inv_OR), 1),
         `2.5 %` = round(if_else(influence == "OR", `2.5 %`, `inv_2.5 %`), 1),
         `97.5 %` = round(if_else(influence == "OR", `97.5 %`, `inv_97.5 %`), 1),
         `OR [IC]` = paste0(OR, " [", `2.5 %`, "; ", `97.5 %`, "]")) %>%
  select(Variable, theme, influence, `OR [IC]`, week) %>%
  pivot_wider(id_cols = c(Variable, theme, influence), names_from = week, values_from = `OR [IC]`, names_prefix = "w") %>%
  View()
```
...
for (wk in c(27, 31, 35, 52)){
  ...
  vacc = unique(vacc) %>%
    filter(codgeo %in% base_codes) %>% 
    mutate(population_carto = as.numeric(population_carto),
           taux_cumu_1_inj = as.numeric(taux_cumu_1_inj)) 
  totPop = sum(vacc$population_carto)
  props = vacc %>%
    group_by(classe_age) %>%
    summarise(proportionPop = sum(population_carto) / totPop) %>%
...
dfocus = read_csv("data/REV_brut.csv") %>%
...
df_patch = dfocus %>%
  ...
  pivot_longer(cols = c(Unemployment_Benef, Interdecile_91_Ratio),
               names_to = "Variable",
               values_to = "quartile") %>%
  filter(!is.na(quartile)) %>%
  mutate(Week = factor(week),
         Variable = if_else(Variable == "Unemployment_Benef", "Unemployment", "Income inequality"))
...
df_patch %>%
  ...
  scale_fill_manual(values = c("#edf8b1", "#7fcdbb", "#1d91c0", "#225ea8")) +
  geom_boxplot() +
  theme_minimal() +
...
# Odds ratios à l'échelle de l'individu
...
pop_matrix = vacct %>%
  filter(!(codgeo %in% c(siren_exclude, 200054781, 200046977, 200054807)) &
           classe_age == "TOUT_AGE" &
           week == 27) %>%
  select(codgeo, population_carto, effectif_cumu_1_inj) %>%
  inner_join(LOG_fact %>% select(codgeo, Overcrowding_rate)) %>%
  inner_join(REV_fact %>% select(codgeo, Unemployment_Benef))

vacc = NULL
over = NULL
unem = NULL
for (i in 1:length(pop_matrix$codgeo)){
  vacc = c(vacc, rep(1, pop_matrix$effectif_cumu_1_inj[i]),
           rep(0, pop_matrix$population_carto[i] - pop_matrix$effectif_cumu_1_inj[i]))
  over = c(over, rep(pop_matrix$Overcrowding_rate[i], pop_matrix$population_carto[i]))
  unem = c(unem, rep(pop_matrix$Unemployment_Benef[i], pop_matrix$population_carto[i]))
}

over_d = as.factor(over)
unem_d = as.factor(unem)
vacc_d = as.factor(vacc)
questionr::odds.ratio(over_d, vacc_d)
questionr::odds.ratio(unem_d, vacc_d)
```
...
```{r}
median(pop_matrix$Overcrowding_rate)
```

```{r}
adresse = "https://datavaccin-covid.ameli.fr/explore/dataset/donnees-de-vaccination-par-epci/download/?format=csv&refine.semaine_injection=2022-01&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B"
data = read_csv2(url(adresse))
data %>%
  group_by(classe_age) %>%
  summarise(vacc1 = sum(effectif_cumu_1_inj, na.rm = T),
            pop = sum(population_carto, na.rm = T)) %>%
  ungroup() %>%
  mutate(taux = 100*vacc1/pop)
```
