---
title: "Données de vaccination et "
...
make_data_covid = function(wk, variable = "taux_cumu_1_inj", classe_age = "tous"){
  ...
  LOG_OR_1107 = questionr::odds.ratio(LOG_1107$`1ry_res`, LOG_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(LOG_1107)[c(2, 4, 6:7, 10:31, 33:36, 40)]) %>%
  ...
  EMP_OR_1107 = questionr::odds.ratio(EMP_1107$`1564_Workforce_rate`, EMP_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(EMP_1107)[c(2:6, 8, 10:13, 19, 21, 23, 25:27, 29, 31:33, 36:53, 55, 57, 59, 63)]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  ...
  FAM_OR_1107 = questionr::odds.ratio(FAM_1107$`SinglePerson_HH`, FAM_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(FAM_1107)[c(2:5, 7:18, 26, 31, 34:39, 45:48)]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  ...
  REV_OR_1107 = questionr::odds.ratio(REV_1107$`D1_st_Living`, REV_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(REV_1107)[c(2:8, 15:16, 18:24, 26)]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  ...
  FOR_OR_1107 = questionr::odds.ratio(FOR_1107$`0205_School`, FOR_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(FOR_1107)[c(2:13, 16, 19, 24:28)]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  ...
  POP_OR_1107 = questionr::odds.ratio(POP_1107$`3044_amg_POP`, POP_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = colnames(POP_1107)[c(4:5, 8:18, 20, 22, 27, 34:35, 38, 40, 50, 53:54)]) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
  ...
  GEO_OR_1107 = questionr::odds.ratio(GEO_1107$latitude, GEO_1107$taux_cumu_1_inj) %>%
    ...
    as.data.frame() %>%
    bind_cols(Variable = c(colnames(GEO_1107)[2:4], "SO-NE", "NO-SE")) %>%
    select(Variable, OR, `2.5 %`, `97.5 %`, p)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/LOG_brut.csv") %>% select(-taux_cumu_1_inj)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/EMP_brut.csv") %>% select(-taux_cumu_1_inj)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/FAM_brut.csv") %>% select(-taux_cumu_1_inj)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/REV_brut.csv") %>% select(-c(taux_cumu_1_inj,
                                                      `Poverty_amg_Owners`,
                                                      `-30_Poverty`,
                                                      `3039_Poverty`,
                                                      `4049_Poverty`,
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/ACT_brut.csv") %>% select(-c(taux_cumu_1_inj,
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/FOR_brut.csv") %>% select(-taux_cumu_1_inj)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/POP_brut.csv") %>% select(-taux_cumu_1_inj)
...
```{r, echo = F, message = F, warning = F}
data_b = read_csv2("data/IMM_brut.csv") %>% select(codgeo,
                                                   Immigrant,
                                                   Immigrant_amg_Men,
                                                   Immigrant_amg_Women)
...
Ces deux tris successifs permettent de réduire le nombre de variables de l'étude à **178 variables**.
...
<!-- === -->
```{r, eval = F, echo = F}
vacc_com = read_csv2(url(paste0("https://datavaccin-covid.ameli.fr/explore/dataset/donnees-de-vaccination-par-commune/download/?format=csv&refine.classe_age=", "00-19", "&refine.semaine_injection=2021-", "35", "&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B"))) %>%
  mutate(codgeo = as.numeric(commune_residence)) %>%
  mutate(taux_cumu_1_inj = as.numeric(taux_cumu_1_inj))

vacc = read_csv2(url(paste0("https://datavaccin-covid.ameli.fr/explore/dataset/donnees-de-vaccination-par-epci/download/?format=csv&refine.semaine_injection=2021-", "35", "&refine.classe_age=", "00-19", "&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B"))) %>%
  mutate(codgeo = as.numeric(epci)) %>%
  mutate(taux_cumu_1_inj = as.numeric(taux_cumu_1_inj)) %>%
  plyr::rbind.fill(vacc_com)
...
test = vacc %>% inner_join(GEO_fact %>%
                             select(codgeo, `NO-SE`, latitude)) %>%
  mutate(taux_cumu_1_inj = binarisation(as.numeric(taux_cumu_1_inj), 0.25),
         `NO-SE` = as.factor(`NO-SE`))

summary(as.factor(test$`NO-SE`))
summary(test$taux_cumu_1_inj)

table(test$`NO-SE`, test$taux_cumu_1_inj)

questionr::odds.ratio(test$`NO-SE`, test$taux_cumu_1_inj)
```
